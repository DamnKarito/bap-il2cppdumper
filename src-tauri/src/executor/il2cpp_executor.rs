...
impl Il2CppExecutor {
    pub fn new(metadata: &Metadata, il2cpp: &mut Il2Cpp) -> Result<Self> {
        ...
        if il2cpp.version >= 27.0 && il2cpp.version < 29.0 {
            let total_count: usize = metadata
                .image_defs
                .iter()
    ...
    pub fn get_type_name(
        ...
        let result =
            self.get_type_name_impl(il2cpp_type, metadata, il2cpp, add_namespace, is_nested);
        ...
        match type_enum {
            Some(Il2CppTypeEnum::SzArray) => {
                ...
                if let Some(element_type) = il2cpp.get_il2cpp_type(element_ptr).cloned() {
                    let element_name =
                        self.get_type_name(&element_type, metadata, il2cpp, add_namespace, false);
            ...
            Some(Il2CppTypeEnum::Ptr) => {
                if let Some(ori_type) = il2cpp.get_il2cpp_type(il2cpp_type.type_ptr()).cloned() {
                    let name =
                        self.get_type_name(&ori_type, metadata, il2cpp, add_namespace, false);
                    return format!("{name}*");
                }
                "void*".to_string()
            }
            Some(Il2CppTypeEnum::Var) | Some(Il2CppTypeEnum::MVar) => {
                if let Some(param) =
                    self.get_generic_parameter_from_type(il2cpp_type, metadata, il2cpp)
                {
                    return metadata
                        .get_string_from_index(param.name_index as i32)
            ...
            | Some(Il2CppTypeEnum::GenericInst) => {
                ...
                let (type_def_opt, generic_class_opt) =
                    if type_enum == Some(Il2CppTypeEnum::GenericInst) {
                        ...
                        let td = self
                            .get_generic_class_type_definition(&gc, metadata, il2cpp)
                            .map(|(td, _)| td);
                        (td, Some(gc))
                    } else {
                ...
                if type_def.declaring_type_index != -1 {
                    if let Some(declaring_type) = il2cpp
                        .types
                        .get(type_def.declaring_type_index as usize)
                        .cloned()
                    {
                        let declaring_name = self.get_type_name(
                            &declaring_type,
                            metadata,
                            il2cpp,
                            add_namespace,
    ...
    pub fn get_type_def_name(
        ...
        if type_def.declaring_type_index != -1 {
            ...
            {
                let declaring_name =
                    self.get_type_name(&declaring_type, metadata, il2cpp, add_namespace, true);
    ...
    pub fn get_generic_container_params(
        ...
        let cache_key = (
            generic_container.generic_parameter_start,
    ...
    pub fn get_method_spec_name(
        ...
        let mut type_name = self.get_type_def_name(
            &type_def,
            type_def_index,
            metadata,
            il2cpp,
    ...
    pub fn try_get_default_value(
        ...
        match type_enum {
            Some(Il2CppTypeEnum::Boolean) => metadata
                .stream
                .read_bool()
                .map(DefaultValue::Bool)
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::U1) => metadata
                .stream
                .read_u8()
                .map(DefaultValue::U8)
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::I1) => metadata
                .stream
                .read_i8()
                .map(DefaultValue::I8)
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::Char) => metadata
                .stream
                .read_u16()
                .map(|v| DefaultValue::Char(char::from_u32(v as u32).unwrap_or('\0')))
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::U2) => metadata
                .stream
                .read_u16()
                .map(DefaultValue::U16)
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::I2) => metadata
                .stream
                .read_i16()
                .map(DefaultValue::I16)
                .map_err(|_| pointer),
            ...
            Some(Il2CppTypeEnum::U8) => metadata
                .stream
                .read_u64()
                .map(DefaultValue::U64)
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::I8) => metadata
                .stream
                .read_i64()
                .map(DefaultValue::I64)
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::R4) => metadata
                .stream
                .read_f32()
                .map(DefaultValue::F32)
                .map_err(|_| pointer),
            Some(Il2CppTypeEnum::R8) => metadata
                .stream
                .read_f64()
    ...
    pub fn get_modifiers(&mut self, flags: u32) -> &str {
        if !self.modifier_cache.contains_key(&flags) {
            ...
            if (flags & 0x0400) != 0 {
                ...
                if (flags & 0x0100) == 0x0000 {
                    result.push_str("sealed override ");
                }
