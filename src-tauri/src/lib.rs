...
fn emit_log(app: &AppHandle, message: &str) {
    let _ = app.emit("dump-log", LogEvent { message: message.to_string() });
}
...
fn request_input(app: &AppHandle, state: &AppState, prompt_type: &str) -> String {
    ...
    let _ = app.emit("dump-input-request", InputRequestEvent {
...
fn try_decrypt_metadata(data: &mut Vec<u8>) -> Option<String> {
    ...
    if key4 != [0u8; 4] && !key4.windows(2).all(|w| w[0] == w[1]) {
        ...
        if is_valid_metadata_version(&test) {
            *data = test;
            return Some(format!("4-byte XOR (key: {:02X}{:02X}{:02X}{:02X})", key4[0], key4[1], key4[2], key4[3]));
        }
    }
    ...
    if key8[0..4] != [0u8; 4] {
        ...
        if test_ver > 0 && test_ver < 200 {
            ...
            if is_valid_metadata_version(&test) {
                *data = test;
                return Some(format!("8-byte XOR (key: {:02X}{:02X}{:02X}{:02X}{:02X}{:02X}{:02X}{:02X})", key8[0], key8[1], key8[2], key8[3], key8[4], key8[5], key8[6], key8[7]));
            }
        }
    }
    ...
    {
        let mut test = data.clone();
        for (i, b) in test.iter_mut().enumerate() { *b ^= key4[i % 4] ^ (i as u8); }
        if &test[0..4] == &magic && is_valid_metadata_version(&test) {
            *data = test;
            return Some(format!("Position-dependent XOR (base key: {:02X}{:02X}{:02X}{:02X})", key4[0], key4[1], key4[2], key4[3]));
        }
    }

    {
        let mut test = data.clone();
        for (i, b) in test.iter_mut().enumerate() { *b ^= key4[i % 4] ^ ((i & 0xFF) as u8); }
        if &test[0..4] == &magic && is_valid_metadata_version(&test) {
            *data = test;
            return Some(format!("Masked position XOR (base key: {:02X}{:02X}{:02X}{:02X})", key4[0], key4[1], key4[2], key4[3]));
        }
    }

    {
        ...
        let mut test = data.clone();
        for i in 0..header_size { test[i] ^= key4[i % 4]; }
        if &test[0..4] == &magic && is_valid_metadata_version(&test) {
            *data = test;
            return Some(format!("Header-only XOR ({header_size} bytes, key: {:02X}{:02X}{:02X}{:02X})", key4[0], key4[1], key4[2], key4[3]));
        }
    }
...
fn detect_unity_version(data: &[u8]) -> Option<String> {
    ...
    let mut i = 0;
    while i + 12 < data.len() {
        let is_20xx = data[i] == b'2' && data[i + 1] == b'0'
            && data[i + 2].is_ascii_digit() && data[i + 3].is_ascii_digit()
            && data[i + 4] == b'.';
        let is_6xxx = data[i] == b'6' && data[i + 1] == b'0'
            && data[i + 2] == b'0' && data[i + 3] == b'0'
            && data[i + 4] == b'.';
        ...
        if (is_20xx || is_6xxx) && data[i + 5].is_ascii_digit() {
            ...
            if candidate.len() >= 8 && candidate.len() <= 20 {
                if let Ok(s) = std::str::from_utf8(candidate) {
                    ...
                    let has_suffix = s.contains('f') || s.contains('b')
                        || s.contains('a') || s.contains('p') || s.contains("rc");
...
fn init_elf(data: Vec<u8>, metadata: &Metadata, config: &Config, app: &AppHandle, state: &AppState) -> error::Result<Il2Cpp> {
    let is_64 = data.len() > 4 && data[4] == 2;
    emit_log(app, &format!("Detected ELF{} format", if is_64 { "64" } else { "32" }));

    let mut elf = Elf::new(data, !is_64)?;
    let version = if config.force_il2cpp_version { config.force_version } else { metadata.version };
    ...
    if config.force_dump || elf.check_dump() {
        ...
        if let Some(addr) = prompt_dump_address(app, state) {
            ...
            elf.is_dumped = true;
            if !config.no_redirected_pointer { elf.load()?; }
    ...
    if force_cr != 0 && force_mr != 0 {
        ...
        if let Some(cr) = code_reg { emit_log(app, &format!("CodeRegistration : 0x{cr:x}")); }
        if let Some(mr) = metadata_reg { emit_log(app, &format!("MetadataRegistration : 0x{mr:x}")); }
        ...
        if !found {
            if let Ok(Some((cr, mr))) = elf.symbol_search() {
                ...
                emit_log(app, &format!("CodeRegistration : 0x{cr:x}"));
                emit_log(app, &format!("MetadataRegistration : 0x{mr:x}"));
                if elf.init(cr, mr).is_ok() { found = true; }
            }
        }
...
fn init_pe(data: Vec<u8>, metadata: &Metadata, config: &Config, app: &AppHandle, state: &AppState) -> error::Result<Il2Cpp> {
    let mut pe = Pe::new(data)?;
    emit_log(app, &format!("Detected PE{} format", if pe.is_32bit { "32" } else { "64" }));
...
fn init_macho_fat(data: Vec<u8>, metadata: &Metadata, config: &Config, app: &AppHandle, state: &AppState) -> error::Result<Il2Cpp> {
    use crate::formats::macho::{parse_fat, extract_fat_slice, MH_MAGIC_64};
    let arches = parse_fat(&data)?;
    emit_log(app, &format!("Detected Fat Mach-O with {} architectures", arches.len()));
    let index = arches.iter().position(|a| a.magic == MH_MAGIC_64).unwrap_or(0);
    emit_log(app, &format!("Auto-selected architecture {} ({})", index + 1, if arches[index].magic == MH_MAGIC_64 { "64-bit" } else { "32-bit" }));
    let slice = extract_fat_slice(&data, &arches[index])?;
    init_macho(slice, metadata, config, app, state)
}

fn init_macho(data: Vec<u8>, metadata: &Metadata, config: &Config, app: &AppHandle, state: &AppState) -> error::Result<Il2Cpp> {
    let magic = read_magic_u32(&data);
    let is_64 = magic == MAGIC_MACHO64;
    emit_log(app, &format!("Detected Mach-O{} format", if is_64 { " 64-bit" } else { " 32-bit" }));
    ...
    if cr_addr == 0 || mr_addr == 0 {
        ...
        if let (Some(cr), Some(mr)) = (helper.find_code_registration(), helper.find_metadata_registration()) {
            cr_addr = cr; mr_addr = mr;
        }
    }
    ...
    let va_segments: Vec<VaSegment> = macho.segments.iter().map(|s| VaSegment { vaddr: s.vmaddr, memsz: s.vmsize, offset: s.fileoff }).collect();
    let mut il2cpp = Il2Cpp::new(macho.stream.clone(), version, macho.is_32bit);
    il2cpp.va_segments = va_segments;
    il2cpp.init(cr_addr, mr_addr, &|addr| macho.map_vatr(addr))?;

    if macho.is_32bit {
        for ptr in il2cpp.method_pointers.iter_mut() { if *ptr > 0 { *ptr -= 1; } }
        for ptr in il2cpp.custom_attribute_generators.iter_mut() { if *ptr > 0 { *ptr -= 1; } }
    }
    Ok(il2cpp)
}

fn init_nso(data: Vec<u8>, metadata: &Metadata, config: &Config, app: &AppHandle, state: &AppState) -> error::Result<Il2Cpp> {
    ...
    if config.force_dump {
        if let Some((cr, mr)) = prompt_manual_addresses(app, state) { cr_addr = cr; mr_addr = mr; }
    }

    if cr_addr == 0 || mr_addr == 0 {
        emit_log(app, "Searching...");
        let method_count = metadata.method_defs.iter().filter(|m| m.method_index >= 0).count();
        let mut helper = nso.get_section_helper(method_count, metadata.type_defs.len(), metadata.metadata_usages_count, metadata.image_defs.len(), version);
        if let (Some(cr), Some(mr)) = (helper.find_code_registration(), helper.find_metadata_registration()) {
            cr_addr = cr; mr_addr = mr;
        } else {
            emit_log(app, "Auto mode failed, requesting manual addresses...");
            if let Some((cr, mr)) = prompt_manual_addresses(app, state) { cr_addr = cr; mr_addr = mr; }
            else { return Err(error::Error::Other("Manual mode cancelled.".into())); }
        }
    }
...
fn init_wasm(data: Vec<u8>, metadata: &Metadata, config: &Config, app: &AppHandle, state: &AppState) -> error::Result<Il2Cpp> {
    ...
    if wasm.check_dump() {
        emit_log(app, "Detected this may be a dump file.");
        if let Some(addr) = prompt_dump_address(app, state) { wasm.stream.image_base = addr; }
    }
    ...
    if cr_addr == 0 || mr_addr == 0 {
        emit_log(app, "Searching...");
        let method_count = metadata.method_defs.iter().filter(|m| m.method_index >= 0).count();
        let mut helper = wasm.get_section_helper(method_count, metadata.type_defs.len(), metadata.metadata_usages_count, metadata.image_defs.len(), version);
        if let (Some(cr), Some(mr)) = (helper.find_code_registration(), helper.find_metadata_registration()) {
            cr_addr = cr; mr_addr = mr;
        } else {
            emit_log(app, "Auto mode failed, requesting manual addresses...");
...
fn run_dump(
    ...
    let final_output = base_dir.join(format!("Dump{dump_num}")).to_string_lossy().to_string();
    ...
    let mut metadata_bytes = fs::read(metadata_path).map_err(|e| format!("Failed to read metadata: {e}"))?;
    ...
    if metadata_magic != MAGIC_METADATA {
        match try_decrypt_metadata(&mut metadata_bytes) {
            Some(scheme) => emit_log(app, &format!("Encrypted metadata detected ({scheme}), decrypting...")),
            None => return Err(format!("Invalid metadata file (magic: 0x{metadata_magic:08X}). Encryption not recognized.")),
        }
    }

    let mut metadata = Metadata::new_with_unity_version(metadata_bytes, unity_version_str.as_deref())
    ...
    Il2CppDecompiler::decompile(&mut executor, &mut metadata, &mut il2cpp, config, &final_output, |msg| {
    ...
    if config.generate_struct {
        ...
        StructGenerator::write_all(&mut executor, &mut metadata, &mut il2cpp, &final_output).map_err(|e| format!("{e}"))?;
        crate::output::embedded_scripts::write_scripts(std::path::Path::new(&final_output)).map_err(|e| format!("{e}"))?;
    ...
    if config.generate_dummy_dll {
        ...
        crate::output::dummy_assembly_generator::generate_dummy_dlls(&mut executor, &mut metadata, &mut il2cpp, config, &final_output).map_err(|e| format!("{e}"))?;
    ...
    if config.generate_generics_dump {
        ...
        if let Err(e) = crate::output::generics::dump_generics(&generics_path.to_string_lossy(), &mut metadata, &mut il2cpp, &mut executor, config) {
            emit_log(app, &format!("Warning: Failed to generate generics_dump.txt: {}", e));
        } else {
...
fn detect_binary(path: String) -> std::result::Result<BinaryInfo, String> {
    ...
    Ok(BinaryInfo { format, unity_version })
}
...
fn start_dump(
    ...
    std::thread::spawn(move || {
        ...
        match result {
            Ok(Ok(output_path)) => {
                let _ = app_for_panic.emit("dump-complete", DumpCompleteEvent {
                    success: true,
                    output_path,
