...
pub fn dump_generics(
    ...
    writeln!(
        file,
        "// IL2CPP Advanced Dump - Runtime Generic Contexts, Attributes & Cross-References"
    )?;
    writeln!(file, "// Generated by Il2CppDumper Rust v3.0")?;
    writeln!(
        file,
        "// ============================================================================\n"
    )?;
    ...
    if config.dump_generics_metadata_usages {
        dump_metadata_usages_section(
            &mut file,
            metadata,
            il2cpp,
            executor,
            &type_def_image_map,
            &method_def_owner_map,
        )?;
    }
...
fn dump_rgctx_section(
    ...
    writeln!(
        file,
        "║  SECTION 1: Runtime Generic Contexts (RGCTX)                                ║"
    )?;
    writeln!(
        file,
        "║  Maps concrete generic instantiations used by the IL2CPP runtime             ║"
    )?;
    writeln!(
        file,
        "╚══════════════════════════════════════════════════════════════════════════════╝\n"
    ...
    for image in &metadata.image_defs.clone() {
        let image_name = metadata
            .get_string_from_index(image.name_index)
            .unwrap_or_default();
        ...
        for type_idx in image.type_start as usize..type_end {
                ...
                executor.get_rgctx_definition_for_type(&image_name, &type_def, metadata, il2cpp)
            {
                if !rgctxs.is_empty() {
                    let type_name = executor
                        .get_type_def_name(&type_def, type_idx, metadata, il2cpp, true, true);
                    ...
                    for (i, rgctx) in rgctxs.iter().enumerate() {
                        ...
                        image_buffer.push_str(&format!(
                            "    [{:3}] {} -> {}\n",
                            i,
                            get_rgctx_type_name(rgctx.rgctx_type),
                            formatted
                        ));
            ...
            for method_idx in type_def.method_start as usize..method_end {
                if method_idx >= metadata.method_defs.len() {
                    break;
                }
                let method_def = metadata.method_defs[method_idx].clone();

                if let Some(rgctxs) = executor.get_rgctx_definition_for_method(
                    &image_name,
                    &method_def,
                    metadata,
                    il2cpp,
                    ...
                    if !rgctxs.is_empty() {
                        ...
                        let img_name = type_def_image_map
                            .get(&type_idx)
                            .map(|(_, n)| n.clone())
                            .unwrap_or_default();
...
fn dump_method_specs_section(
    ...
    writeln!(
        file,
        "║  SECTION 2: Instantiated Generic Methods (MethodSpecs)                      ║"
    )?;
    writeln!(
        file,
        "║  Every concrete generic method instantiation with its native address         ║"
    )?;
    writeln!(
        file,
        "╚══════════════════════════════════════════════════════════════════════════════╝\n"
...
fn dump_custom_attributes_section(
    ...
    writeln!(
        file,
        "║  SECTION 3: Custom Attributes                                               ║"
    )?;
    writeln!(
        file,
        "║  Reconstructed [SerializeField], [Header], [Range] etc. for every type       ║"
    )?;
    writeln!(
        file,
        "╚══════════════════════════════════════════════════════════════════════════════╝\n"
    ...
    for (img_idx, image) in metadata.image_defs.clone().iter().enumerate() {
        ...
        for type_idx in image.type_start as usize..type_end {
            let type_def = metadata.type_defs[type_idx].clone();
            let type_name =
                executor.get_type_def_name(&type_def, type_idx, metadata, il2cpp, true, true);
            let mut type_buffer = String::new();
            let mut type_has_attrs = false;

            let type_attrs = collect_attributes(
                metadata,
                img_idx,
            ...
            for field_offset in 0..type_def.field_count as usize {
                ...
                if field_idx >= metadata.field_defs.len() {
                    break;
                }
                let field_def = metadata.field_defs[field_idx].clone();
                let field_attrs = collect_attributes(
                    metadata,
                    img_idx,
            ...
            for method_idx in type_def.method_start as usize..method_end {
                ...
                let method_def = metadata.method_defs[method_idx].clone();
                let method_attrs = collect_attributes(
                    metadata,
                    img_idx,
            ...
            for prop_offset in 0..type_def.property_count as usize {
                ...
                if prop_idx >= metadata.property_defs.len() {
                    break;
                }
                let prop_def = metadata.property_defs[prop_idx].clone();
                let prop_attrs = collect_attributes(
                    metadata,
                    img_idx,
                    prop_def.custom_attribute_index,
                    prop_def.token,
                );
                if !prop_attrs.is_empty() {
                    let prop_name = metadata
                        .get_string_from_index(prop_def.name_index)
                        .unwrap_or_else(|_| "?".to_string());
                    type_buffer
                        .push_str(&format!("    [Property] {} -> {}\n", prop_name, prop_attrs));
                    type_has_attrs = true;
                    attr_count += 1;
                }
            }
...
fn collect_attributes(
    ...
    if metadata.version >= 29.0 {
        ...
        for j in 0..range.count as usize {
            ...
            if type_idx >= metadata.attribute_types.len() {
                break;
            }
...
fn dump_string_literals_section(file: &mut BufWriter<File>, metadata: &mut Metadata) -> Result<()> {
    writeln!(
        file,
        "╔══════════════════════════════════════════════════════════════════════════════╗"
    )?;
    writeln!(
        file,
        "║  SECTION 4: String Literal Table                                            ║"
    )?;
    writeln!(
        file,
        "║  Every hardcoded string in the game binary with its index                    ║"
    )?;
    writeln!(
        file,
        "╚══════════════════════════════════════════════════════════════════════════════╝\n"
...
fn dump_metadata_usages_section(
    ...
    writeln!(
        file,
        "║  SECTION 5: Metadata Usage Cross-References                                 ║"
    )?;
    writeln!(
        file,
        "║  Maps binary addresses to the types, methods, fields and strings they use    ║"
    )?;
    writeln!(
        file,
        "╚══════════════════════════════════════════════════════════════════════════════╝\n"
...
fn dump_vtable_section(
    ...
    writeln!(
        file,
        "║  SECTION 6: Virtual Method Tables (VTables)                                  ║"
    )?;
    writeln!(
        file,
        "║  Full virtual dispatch tables showing which methods override which slots      ║"
    )?;
    writeln!(
        file,
        "╚══════════════════════════════════════════════════════════════════════════════╝\n"
    ...
    for (_img_idx, image) in metadata.image_defs.clone().iter().enumerate() {
        ...
        for type_idx in image.type_start as usize..type_end {
            ...
            let parent_name = if type_def.parent_index >= 0
                && (type_def.parent_index as usize) < il2cpp.types.len()
            {
                ...
                if usage == 6 {
                    if (index as usize) < il2cpp.method_specs.len() {
                        ...
                        let (spec_type_name, spec_method_name) =
                            executor.get_method_spec_name(index as usize, metadata, il2cpp, true);
                    ...
                    if let Some(md) = metadata.method_defs.get(index as usize).cloned() {
                        if md.slot != 0xFFFF {
                            ...
                            let declaring_name = if let Some(td) =
                                metadata.type_defs.get(declaring_type_idx).cloned()
                            {
                                executor.get_type_def_name(
                                    &td,
                                    declaring_type_idx,
                                    metadata,
                                    il2cpp,
                            ...
                            let method_name = metadata
                                ...
                                .unwrap_or_else(|_| "?".to_string());
                            let img_name = type_def_image_map
                                .get(&declaring_type_idx)
                                .map(|(_, n)| n.clone())
                                .unwrap_or_default();
                            let method_ptr = il2cpp.get_method_pointer(&img_name, &md);
                            let rva = if method_ptr > 0 {
                                il2cpp.get_rva(method_ptr)
                            } else {
                                0
                            };
    ...
    writeln!(
        file,
        "Total types with VTables: {} ({} total slots)\n",
...
fn dump_interface_section(
    ...
    writeln!(
        file,
        "║  SECTION 7: Interface Implementation Tables                                  ║"
    )?;
    writeln!(
        file,
        "║  Which interfaces each class implements and at what vtable offsets            ║"
    )?;
    writeln!(
        file,
        "╚══════════════════════════════════════════════════════════════════════════════╝\n"
    ...
    for (_img_idx, image) in metadata.image_defs.clone().iter().enumerate() {
        ...
        for type_idx in image.type_start as usize..type_end {
            let type_def = metadata.type_defs[type_idx].clone();
            if type_def.interfaces_count == 0 && type_def.interface_offsets_count == 0 {
                continue;
            }
            ...
            if type_def.interfaces_count > 0 {
                ...
                for i in 0..type_def.interfaces_count as usize {
                    ...
                    if iface_idx >= metadata.interface_indices.len() {
                        break;
                    }
                    let interface_type_index = metadata.interface_indices[iface_idx];
                    if interface_type_index >= 0
                        && (interface_type_index as usize) < il2cpp.types.len()
                    {
                        let iface_type = il2cpp.types[interface_type_index as usize].clone();
                        let iface_name =
                            executor.get_type_name(&iface_type, metadata, il2cpp, true, false);
                        type_buffer.push_str(&format!("      - {}\n", iface_name));
            ...
            if type_def.interface_offsets_count > 0 {
                ...
                for i in 0..type_def.interface_offsets_count as usize {
                    ...
                    if offset_idx >= metadata.interface_offsets.len() {
                        break;
                    }
                    let iface_offset = metadata.interface_offsets[offset_idx].clone();
                    if iface_offset.type_index >= 0
                        && (iface_offset.type_index as usize) < il2cpp.types.len()
                    {
    ...
    writeln!(
        file,
        "Total types with interfaces: {} ({} total implementations)\n",
...
fn dump_legacy_metadata_usages(
            ...
            match *usage_type {
                ...
                2 => {
                    if src < il2cpp.types.len() {
                        ...
                        let type_name =
                            executor.get_type_name(&type_ref, metadata, il2cpp, true, false);
                        type_ref_entries
                            .push(format!("  0x{:08X} | {}_var (Il2CppType*)", rva, type_name));
                    }
                }
                3 => {
                    if let Some(method_def) = metadata.method_defs.get(src).cloned() {
                            ...
                            .cloned()
                        {
                            ...
                            let type_name = executor
                                .get_type_def_name(&type_def, td_idx, metadata, il2cpp, true, true);
                            let method_name = metadata
                                .get_string_from_index(method_def.name_index as i32)
                                .unwrap_or_else(|_| "?".to_string());
                            let img_name = type_def_image_map
                            ...
                            method_def_entries.push(format!(
                                "  0x{:08X} | {}.{}(){}",
                                rva, type_name, method_name, target_str
                            ));
                        }
                    }
                }
                            ...
                            if let Some(td) = metadata.type_defs.get(klass_idx) {
                                ...
                                if let Some(fd) = metadata.field_defs.get(field_idx) {
                                    let field_name = metadata
                                        ...
                                        .unwrap_or_default();
                                    field_ref_entries.push(format!(
                                        "  0x{:08X} | {}.{}",
                                        rva, type_name, field_name
                                    ));
                                }
                            }
                        }
                ...
                5 => {
                    if let Ok(string_literal) = metadata.get_string_literal_from_index(src) {
                        ...
                        string_lit_entries
                            .push(format!("  0x{:08X} | [{}] \"{}\"", rva, src, truncated));
                    }
                }
                6 => {
                    if src < il2cpp.method_specs.len() {
                        ...
                        method_spec_entries.push(format!(
                            "  0x{:08X} | {}.{}(){}",
                            rva, spec_type_name, spec_method_name, target_str
                        ));
                    }
                }
                _ => {}
    ...
    write_usage_subsection(
        file,
        "Type Info References (Il2CppClass*)",
        &type_info_entries,
    )?;
    ...
    write_usage_subsection(
        file,
        "Generic Method (MethodSpec) References",
        &method_spec_entries,
    )?;

    let total = type_info_entries.len()
        + type_ref_entries.len()
        + method_def_entries.len()
        + field_ref_entries.len()
        + string_lit_entries.len()
        + method_spec_entries.len();
...
fn dump_v27_metadata_usages(
    ...
    for sec in &data_sections {
        let sec_end =
            std::cmp::min(sec.offset_end, il2cpp.stream.len() as u64).saturating_sub(pointer_size);
            ...
            match usage {
                ...
                5 => {
                        ...
                        metadata.get_string_literal_from_index(decoded_index as usize)
                    {
                        ...
                        string_lit_entries.push(format!(
                            "  0x{:08X} | [{}] \"{}\"",
                            rva, decoded_index, truncated
                        ));
                    }
                }
                6 => {
...
fn format_rgctx(
    ...
    if t == 1 || t == 2 || t == 4 {
        ...
        if method_idx >= 0 && (method_idx as usize) < il2cpp.method_specs.len() {
            let (t_name, m_name) =
                executor.get_method_spec_name(method_idx as usize, metadata, il2cpp, true);
